    ...
    try:
        while has_more:
            ...
            async with c.requests_async(url=url) as r:
                ...
                # The batch may span the boundary (some messages within range,
                # some older).  Yield the in-range slice before stopping so we
                # don't lose messages that fall inside the after–before window.
                if min_ts < after:
                    in_range = [
                        x for x in batch
                        if float(
