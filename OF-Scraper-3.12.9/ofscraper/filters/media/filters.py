...
from ofscraper.db.operations_.media import (
    ...
    get_media_post_ids_downloaded_only,
)
...
def final_media_sort(media):
    ...
    is_desc = read_args.retriveArgs().media_desc
    log.debug(f"Using download sort {media_sort}")
    if media_sort == "random":
        random.shuffle(media)
    elif media_sort == "date":
        media = sorted(media, key=lambda x: x.date, reverse=bool(is_desc))
    elif media_sort == "text":
        media = sorted(media, key=lambda x: x.text, reverse=bool(is_desc))
    elif media_sort == "filename":
        media = sorted(media, key=lambda x: x.filename, reverse=bool(is_desc))
    return media
...
def previous_download_filter(medialist, username=None, model_id=None):
    ...
    if read_args.retriveArgs().force_all:
        ...
        if allow_dupe_downloads:
            media_pairs = set(
                get_media_post_ids_downloaded_only(model_id=model_id, username=username)
                or []
            )
            log.debug(
            ...
            medialist = [
                m
                for m in medialist
                if (m.id, getattr(m, "postid", None)) not in media_pairs
            ]
        else:
            media_ids = set(
        ...
        if allow_dupe_downloads:
            ...
            log.debug("Number of downloaded media/post pairs in database for model")
            medialist = [
                m
                for m in medialist
                if (m.id, getattr(m, "postid", None)) not in media_pairs
