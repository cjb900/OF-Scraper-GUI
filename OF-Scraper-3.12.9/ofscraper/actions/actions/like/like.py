...
def filter_for_unfavorited(posts: list) -> list:
    # Only like posts that are open and NOT already favorited.
    # Some Post objects may not expose `favorited`; treat missing/None as "not favorited".
    output = list(
        filter(
            lambda x: bool(getattr(x, "opened", False))
            and (getattr(x, "favorited", False) is False),
            posts,
        )
    )
...
def filter_for_favorited(posts: list) -> list:
    # Only unlike posts that are open and already favorited.
    output = list(
        filter(
            lambda x: bool(getattr(x, "opened", False))
            and (getattr(x, "favorited", None) is True),
            posts,
        )
    )
